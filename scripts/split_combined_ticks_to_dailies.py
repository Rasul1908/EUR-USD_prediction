"""
Recreate daily tick Parquets from one combined file (after dailies were deleted).

The notebook merge writes row groups in chronological order, so each UTC calendar
day appears in one contiguous run. This script streams the combined file in batches
and writes EURUSD_YYYY-MM-DD_ticks_utc.parquet files.
...
If the combined file is missing, re-run notebooks/ingest_dukascopy.ipynb for your
date range (set DELETE_DAILY_AFTER_MERGE = False until you are done iterating).
"""
...
def default_combined_path(out_dir: Path) -> Path:
    ...
    if not matches:
        raise FileNotFoundError(
            ...
            "Pass --combined explicitly or restore the file from backup."
        )
    if len(matches) > 1:
        raise RuntimeError(
